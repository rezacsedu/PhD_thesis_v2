...
\hspace*{3.5mm} As the global interpretability signifies overall transparency of a model on an abstract level, a surrogate model is often used to explain it's global behaviour by approximating the behaviour of the original model. Using an oracle or a surrogate model, we answer following questions: 
...
Feature interactions among globally important features with attention mechanism are demonstrated in \cref{fig:global_feature_impacts}. Global feature impacts signify which features have positive impact and which features have negative impact on the output of the diagnosis decision. GBP4, VPS9D1-AS1, SLN, SEMA3E, GP2, CXCL10, RAVER1, AL603825-1, TBC1D4, CLTRN, OASL, CWH43, and GABRP positively impact the prediction. This also signifies that if the prediction is in favour of specific cancer type~(let's say COAD) for a specific patient, these variables will play crucial role in maintaining this prediction. However, if we look at TRIM17, CDS1, PCOLCE2, MGP, MTCO1P53, TFF3, AC026403-1, HAGLROS, LAPTM5, SULT4A1, EN1, and EFNB1 have negative impact on the prediction. This means that if the prediction is "COAD" and we increase the value of these variables, we will likely end up flipping-up the prediction to another cancer type.
