...
\hspace*{3.5mm} Although, permutation feature importance  provides a view on global feature importance and signifies how important a feature is for a particular model, it does not necessarily reflect to the intrinsic predictive value of a feature by itself. In other words, there is a possibility that a feature that seems to have lower importance for a under/overfitted model could be very important for a better fitted model. Nevertheless, order in which a model sees features can affect the predictions. Therefore, using SHAP, the computation is repeated in all possible orders to compare the features fairly. 

\hspace*{3.5mm} If $x_i$ is a feature in modality specific input $X_k$, it's importance w.r.t Shapley value $\phi$ is computed by comparing what the model predicts with and without the $x_i$ from all possible combinations of $n$ features in $X_k$. For $x_i$, prediction $\hat x_i$ and Shapley value $\phi$ are then calculated, as follows~\cite{NIPS2017_7062}. 
...
\hspace*{3.5mm} If $x_i$ has no or almost zero effect on the predicted value is expected to produce a Shapley value of 0. If two features $x_i$ $x_{i+1}$ contribute equally to the prediction, Shapley values should be the same~\cite{NIPS2017_7062}. 
...
\paragraph{Stage-1: individual modality}\hspace{-3mm} of MCAE represents a specific modality for each type of data. Individual modality CAE is not only a one-layer CAE, but also a multilayer and gradually shrinking CAE with different number of layers per modality. An CAE consist of two sub-networks called encoder and decoder.  Encoder is a sigmoid function $g(.)$ parameterized by $\phi$, while decoder function $f(.)$ is parameterized by $\theta$. For input $X_{k} \in \mathbb{R}^{D}$ for each of $k \in \mathbb{R}^K$ modalities, encoder encodes the input into a lower-dimensional code, such that $X_{k}$ is mapped and transformed into embedding space~($Z_k$). If $p$ and $n$ are number of hidden and input units, latent-space representation $Z_k=g_{\phi}(X_{k})$ is learned in the bottleneck layer~\cite{mmdcae}: 

\begin{equation}
    Z_k = h_{k}=g_\phi \left({X}_{k}\right)=\sigma\left(W_{k} * X_{k}+b_{k}\right)
\end{equation}

\hspace*{3.5mm} where $\phi$ are trainable parameters, which includes the weight matrix $W_{k} \in \mathbb{R}^{n \times p}$ and the bias vector $b_{k} \in \mathbb{R}^{n}$ that are specific to respective modality $k$, and $\sigma$ is the exponential linear unit~(ELU)~\cite{clevert2015fast} activation function\footnote{Unlike MAE architecture, where ReLU activation function was used.}, which is defined as~\cite{clevert2015fast}: 
\begin{equation}
...
\hspace*{3.5mm} where $\alpha_r$ is the regularization weights. The decoder tries to reconstruct from $h_k$, original input $X_{k}$ from the latent representation $Z_k$ using the decoder function $f(.)$. Hidden representation $h_{k}$ is mapped back as reconstructed version $\hat{X}_{k}$, similar to original input ${X}_{k}$, as follows~\cite{mmdcae}: 
...
\hspace*{3.5mm} where $\theta$ are trainable parameters, including weight matrix $\hat W_{k} \in \mathbb{R}^{n \times p}$ and bias vector $\hat b_{k}$, that are specific to modality $k$, and $\Psi$ is sigmoid activation function. Distance measure~($d_{k}$) that quantifies difference between input $X_{k}$ and network's output $f_\theta(X_{k})$, is a typical reconstruction loss~\cite{KarimIEEEAccess2019}: 
...
\paragraph{Stage-2: cross-modality}\hspace{-3mm} is also a multilayer gradually shrinking CAE, but with different output layer size for each prediction. However, number of layers and units per layer in both encoder and decoder are symmetric.  Latent representations of all modalities are concatenated into a single representation~\cite{liu2016multimodal}, such that the dimensionality of individual modalities $\phi$ is the same:  

\begin{equation}
    h_{mcae}=\sigma\left(W_{mcae}\left[h_{m} \oplus h_{e} \oplus h_{r}\right]+b_{mcae}\right),
\end{equation}

\hspace*{3.5mm} where $\oplus$ is the concatenation operation. Whole MCAE is pre-trained to reconstruct individual modalities from the hidden representation $h_{mcae}$~\cite{liu2016multimodal}:

\begin{equation}
    \left[\hat{h}_{m}\oplus \hat{h}_{e} \oplus \hat{h}_{r}
    \right]=\Psi\left(\hat W_{mcae} h_{mcae}+\hat {b}_{mcae}\right)
\end{equation}

\hspace*{3.5mm} The decoder module, which has a similar gradual increasing architecture as an encoder, reconstructs the original representation of individual modalities, $\hat{X}_{c}$, $\hat{X}_{e}$, and $\hat{X}_{r}$ as~\cite{wang2018associativemulti}: 

\begin{equation}
    \begin{aligned}
        \hat{X}_{c} &=\Psi\left(\hat W_{c} \hat{h}_{m}+\hat{b}_{m}\right) \\
...
Approaches based on multimodal learning directly merge all the latent representations into a single vector representation. As there is not huge difference between GE, miRNA, or CNV values. Therefore, merging them blindly could ignore subtle differences between different input modalities w.r.t feature contributions and may increase dimensions of the representation. However, a concatenated vector representation from MCAE is not easily interpretable. This critical drawback impedes the interpretability, making the MCAE a black-box model. On the other hand, disentangling latent vectors can provide insight into what features did the representations capture and what attributes of the samples are the classification based on~\cite{karimTCBB2020}. 
