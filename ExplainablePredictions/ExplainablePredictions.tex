...
Nevertheless, latent feature vectors are not easily interpretable. This critical drawback impedes the interpretability, making the MCAE a black-box model. However, disentangling latent vectors can provide insight into what features did the representations capture and what attributes of the samples are the classification going to be based on~\cite{karimTCBB2020}. Unlike the unimodal CNN models, where fully convolutional layers act as feature extractor, it is more reasonable to let the MCAE model to perform feature selection. One of the widely used techniques is called attention mechanism, which is used for computing feature importance. 
...
For $\left(\boldsymbol{x}_{i}, \boldsymbol{y}_{i}\right), 1 \leq i \leq n$ samples, instance-level aggregations is about computing the attention map $\operatorname{SAN}\left(\boldsymbol{x}_{i}\right)$, for $i$ -th instance as~\cite{vskrlj2020feature}: 

%Let $\operatorname{SAN}\left(\boldsymbol{x}_{i}\right)$ represent the attention output of the $i$ -th instance, defined as
\begin{equation}
    \operatorname{SAN}\left(\boldsymbol{x}_{i}\right)=\frac{1}{k} \bigoplus_{k}\left[\operatorname{softmax}\left(W_{l_{\mathrm{att}}}^{k} \boldsymbol{x}_{i}+\boldsymbol{b}_{l_{\mathrm{att}}}\right)\right]
\end{equation}

\hspace*{3.5mm} Then the outputs are averaged on-instance basis, to get the global feature importance~\cite{vskrlj2020feature}: 

\begin{equation}
    R_{I}=\frac{1}{n} \sum_{i=1}^{n} \operatorname{SAN}\left(\boldsymbol{x}_{i}\right)
\end{equation}
...
The global attention-based feature computation is extended for multimodal convolutional autoencoder architecture, such that the attention vector~(that represents individual feature weights) for each input modality contribute to the latent space $Z_k$.  
