...
Before, defining the problem formally, we introduce the notations of cancer types and subtypes in the context of a DSS for cancer diagnosis: 
...
\hspace*{3.5mm} 
%For example, a CNV sample $x$ consists of a set of $m$ attribute-value pairs $\left(a_{i}, v_{i}\right)$, where $a_i$ is a feature and $v_i$ is its value from $a_{i}$, where both $a_{i}$ and $v_i \in \mathbb{R}^{D}$. 
The problem of classifying the patients is grouping into a specific cancer types based on their unimodal or multimodal genomic profiles. In other words, the cancer typing method can be formulated as a prediction in a multi-class classification setting. We consider the following input modalities for the proposed DSS:    
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Multimodality} - refers to the way to integrate multiple input modalities~\cite{mmsurvey}.
\end{itemize}  

\hspace*{3.5mm} Let $X$ = $\left(x_{1}, x_{2}, \ldots, x_{n}\right)$ be a genomic dataset of $n$ samples, where a sample $x_i$ composed of $m$ independent genes $G$ = $\left(g_{1}, g_{2}, \ldots, g_{m}\right)$, $X \in \mathbb{R}^{D}$, and $G \in \mathbb{R}^{D}$. In other words, sample $x_i$ consists of a set of $m$ attribute-value pairs $\left(a_{i}, v_{i}\right)$, where $a_i$ is a feature and $v_i$ is its value, such that both $a_{i}$ and $v_i \in \mathbb{R}^{D}$. To solve the multiclass-classification problem, first we train a DNN architecture and generate a robust `black-box' model, instead of relying on interpretable models. 

\hspace*{3.5mm} However, in case of additional input modalities~(i.e., another types of genomic data), it will contribute as another input modality towards multimodality, where to generate the modality specific latent representation of the input modalities. From a given multimodal genomic dataset $D=\{X_1, X_2,..., X_k\}$ about $n$ patients, we can consider classifying an individual $x_i$ into a specific cancer type based on his or her genomic profile, where %$x_k \in \mathbb{R}^{D}$,
where $X_k$ = ${\mathbf{\{x_1,x_2,..., x_n}}\}$ is represent an individual input modality. 

\hspace*{3.5mm} Since each modality is comprising very high dimensional genomic data, we embedded each input modality $x_i$ into lower dimensional feature space instead of classifying them using their original representation. In other words, using learned representation from individual modalities, we train a `black-box' DNN model, as shown in \cref{fig:chapter_2_wf}, such that we transform each input modality $X$ with a nonlinear mapping $f_{\theta}: x_i \rightarrow z_i$, where $\theta$ are learnable parameters and $z_i \in \mathbb{R}^{K}$ is the learned embedding in which $K \ll D$. 
