...
\hspace*{3.5mm} Ontological reasoning can be can be applied in the form of symbolic reasoning to infer more complex rules~\cite{futia2020integration}. Suppose a man named John is very hungry. Assuming John likes pizza very much, but he is a vegetarian. John went to a nearby restaurant and asked the waiter if they have an option for the vegetarian, preferably a vegetable pizza. Suppose, the waiter replied saying ``I'm not sure if we have an option for the vegetarian. Margherita pizza could be suitable for the vegetarian''. 
%How will you be sure that the waiter is right? Well, the simplest answer to this question would be 
After reading the food ingredients and based on human-instincts, John can come up with a logical conclusion that the waiter is right, as shown in \cref{fig:sparqlEx11}\footnote{Steps are described in natural human language.)}.   
...
\hspace*{3.5mm} However, for a machine deriving such a logical conclusion always subject to the availability of prior facts. However, if we have a domain-specific ontology, an ontology reasoner can infer, based on a set of prior facts and evidence, a logical conclusion in the form of description logic axioms, as shown in \cref{fig:pizza11}.  To reach a logical conclusion, deductive reasoning and inductive reasoning are two widely used technique. %, which we will see in the next subsection. 
...
Subsequently, research initiatives are gradually adopting semantic web~(SW) technologies~\cite{karim2018improving} such as knowledge bases~(KBs) and domain-specific ontologies as the means of building structured networks of interconnected knowledge~\cite{futia2020integration}. Such knowledge structure not only contain data, but also metadata and domain knowledge~(ontologies containing axioms or rules), all in the same uniform structure, and are then called knowledge graphs\footnote{Ontology + data = knowledge graph.}~(KGs)~\cite{wilcke2017knowledge,hogan2020knowledge}. For example, Shen et al.~\cite{POSTF} has shown that some genes have both oncogenic and tumor-suppressor functionality called proto-oncogenes with tumor-suppressor function~(POTSF). Most of the POTSF genes act as transcription factors or kinases and exhibit dual biological functions, e.g., they both positively and negatively regulate transcription in cells~\cite{POSTF}. There are some specific cancer types, e.g., leukemia are over-represented by such POTSF genes, whereas some common cancer types like lung cancer are under-represented by them~\cite{POSTF}. 
...
\subsection{Semantic web technologies in XAI}
...
\hspace*{3.5mm} A semantic reasoner infers and produces new knowledge from existing knowledge to answer interactive queries.
