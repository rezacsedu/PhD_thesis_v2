...
\hspace*{3.5mm} On the other hand, symbols are also often express lessons that we derive inductively based on domain knowledge or real-world experiences. That is, inductive reasoning is the opposite of deductive reasoning in which a broad generalizations is made from specific observations~\cite{hogan2020knowledge}. In real-life, human make many observations, discern a pattern, make a generalization, and infer an explanation or a theory. Similarly, using inductive reasoning, specific observations can be made to draw a general conclusion. For example, \textit{``Oncogenes are responsible for cancer, don't apply gene silencing blindly''}. 
However, the downside of inductive reasoning is that it can end up with a false conclusion, even if all of the premises are true in a statement. For example, \textit{``TCF3 is an oncogene; TCF3 has tumor-suppressor functionality; all oncogenes have tumor-suppressor functionality''}. Since only the proto-oncogenes have tumor-suppressor functionality, this conclusion is not biologically true.
...
\hspace*{3.5mm} In the symbolic AI approach~(also known as Good Old-Fashioned Artificial Intelligence~(GOFAI) - a dominant paradigm of 1980s), booth deductive and inductive reasoning techniques are used in which symbols are elements of a \textit{`lingua franca'} between humans and neural networks~\cite{janssens2007dynamic}. However, since contemporary explainability techniques mostly adjust the weights and measures model inputs to determine their effects on outputs, many AI practitioners merely consider symbolic AI systems as the first step towards achieving a true XAI system. 
...
\begin{sidewaysfigure}
	...
	\caption[Symbolic vs. neuro-symbolic AI]{A hybrid approach, conceptually recreated based on the by Anil A. et al~\cite{AIsnextbigleap} is known as neuro-symbolic AI. A neuro-symbolic AI system combines both symbolic AI~(upper left - where a human develops ``knowledge bas'' that the system uses to answer questions) and neural networks~(upper right) that are trained to provide the answers. During training, a DNN adjusts the strength of the connections between layers of nodes. In a hybrid AI system, human is replaced with a DNN~(but human-in-the-loop still maybe necessary), to generate only those portions of the knowledge base that it needs to answer for a given question.}
...
\hspace*{3.5mm} However, extracting knowledge and facts from unstructured, heterogeneous, and scattered sources is a very challenging task. Information extraction is the process of automatically extracting structured knowledge and facts from such unstructured and/or semi-structured machine-readable documents and other electronically represented sources~\cite{Liddy.2001}. A shown in \cref{fig:IE_pipeline}, information extraction is typically divided into named entity recognition~(NER), entity linking, and relation extraction. Relation extraction also involves relation classification, which is typically formulated as a classification problem to classify the relationship between the entities identified in the text~\cite{xue2019fine}. A classifier takes a piece of text and two entities as inputs and predicts possible relations between the entities as output. 
...
This section analyses related works focusing on symbolic and neuro-symbolic reasoning, semantic web~(SW) technologies, and interactive explanations. Although some of the discussed works are not directly applicable, they still provide the foundations for developing the decision reasoning module for the DSS.

\subsection{Symbolic and hybrid AI}
...
Research initiatives are gradually adopting SW technologies~\cite{karim2018improving} such as knowledge bases~(KBs) and domain-specific ontologies as the means of building structured networks of interconnected knowledge~\cite{futia2020integration}. Hogan et al.~\cite{hogan2020knowledge} has provided a comprehensive review of articles of knowledge graphs, covering knowledge graph creation, enrichment, quality assessment, and refinement. Apart from these literature that focus on the theoretical concepts, several large-scale KGs have been constructed either by manual annotation, crowd-sourcing~(e.g., DBpedia) or by automatic extraction from unstructured data~(e.g., YAGO\footnote{\url{https://github.com/yago-naga/yago3}})~\cite{wang2015explicit} targeting KG analytics for specific use cases. 
...
Bidirectional Encoder Representations from Transformers~(BERT)~\cite{devlin2018bert} is a language model that utilizes bidirectional attention mechanism and large-scale unsupervised corpora to obtain effective context-sensitive representations of words in a sentence~\cite{xue2019fine}. Subsequently, several variants of BERT are proposed and widely used as transformer language models for a variety of downstream NLP tasks like NER. 

\hspace*{3.5mm} Sun et al.~\cite{sun2020biomedical} employed transformer for predicting entity labels, where word representations are obtained from a pre-trained BERT model. BERT-based approaches have also been applied to scientific texts~(e.g, SciBERT~\cite{Beltagy2019SciBERT}) and adapted in other domain such as ChemBERTa~\cite{chithrananda2020chemberta} in chemical engineering for molecular property prediction.
...
\begin{figure*}
	\centering
	\includegraphics[scale=0.6]{images/ner_stages_ex.png}
	\caption{Knowledge extraction example from text, where new knowledge are shown in dashed boxes.}
...
\begin{tikzpicture}
    %\node[task] (tokenL) {Input text:};
    %TP53 is an oncogene, which is responsible for breast cancer and has POTSF functionality.
    %TP53 is responsible for breast cancer and has POTSF functionality in PubMed.
    \node[task] (tokenL) {Pre-processing:};
    \node[right=1cm of tokenL.east,anchor=west,split,rectangle split parts=17] (token) {\nodepart{one} TP53 \nodepart{two} is \nodepart{three} responsible \nodepart{four} for \nodepart{five}Breast \nodepart{six} Cancer \nodepart{seven} and \nodepart{eight} has \nodepart{nine} POTSF \nodepart{ten} functionality \nodepart{eleven}, \nodepart{twelve} which \nodepart{thirteen} is \nodepart{fourteen} mentioned \nodepart{fifteen} in \nodepart{sixteen} PubMed \nodepart{seventeen}.};
    
    \node[task,below=\tgap of tokenL.south] (nerL) {\begin{tabular}{@{}c@{}}Named Entity\\Recognition:\end{tabular}};
    
    \node[word] at (nerL-|token.one south) (santiagoNER) {TP53};
    
    \node[word,xshift=-0.7ex] at (nerL-|token.six) (rnNER) {Breast Cancer};
    
    \node[word,xshift=-1ex] at (nerL-|token.thirteen south) (whsNER) {POTSF};
    
    \node[word] at (nerL-|token.sixteen south) (yearNER) {PubMed};
    
    \node[task,below=0.6\tgap of nerL.south] (elL) {\begin{tabular}{@{}c@{}}Entity Linking:\end{tabular}};
    
    \node[iri] at (elL-|santiagoNER) (santiagoEL) {TP53};
    
    \node[iri] at (elL-|rnNER) (eiEL) {Breast Cancer};
    
    \node[iri,dashed] at (elL-|whsNER) (whsEL) {POTSF};
    
    \node[iri,dashed] at (elL-|yearNER) (yearEL) {PubMed};
    
    %\node[task,below=\tgap of elL.south] (wsdL) {\begin{tabular}{@{}c@{}}Word Sense\\Disambiguation:\end{tabular}};
    %
    %\node at (wsdL-|token.three south) (flightWSD) {\AricaAirport};
    %
    %\node at (wsdL-|token.ten south) (namedWSD) {\faFlagCheckered};
    
    \node[task,below=1.5\tgap of elL.south] (reL) {\begin{tabular}{@{}c@{}}Relation Extraction\\\scriptsize \textit{Binary}:\end{tabular}};
    
    \node[iri,yshift=-1ex] at (reL-|santiagoEL) (santiagoRE) {TP53};
    
    \node[iri] at (santiagoRE-|eiEL) (eiRE) {Breast Cancer}
      edge[arrin] node[lab] {responsibleFor} (santiagoRE);
    
    \node[iri,dashed] at (santiagoRE-|whsEL) (whsRE) {POTSF}
      edge[arrin] node[lab] {hasType} (eiRE);
    
    \node[iri,dashed] at (santiagoRE-|yearEL) (yearRE) {PubMed}
     edge[arrin] node[lab] {hasEvidence} (eiRE);
    
    \node[task,below=1.5\tgap of reL.south] (neL) {\begin{tabular}{@{}c@{}}\scriptsize \textit{$n$-ary}:\end{tabular}};
    
    \node[between=eiRE and whsRE] (mid) {};
    
    \node[bnode,dashed] at (neL-|mid) (nary) {}
      edge[arrout,bend left=10] node[lab] {patient} (eiRE)
      edge[arrout,bend right=10] node[lab] {name} (whsRE)
      edge[arrout,bend right=15] node[lab] {time} (yearRE);
    \end{tikzpicture}
    
    \caption{Text extraction example; nodes new to the knowledge graph are shown dashed \label{fig:textExtract}}
\end{figure}
\fi 
...
\hspace*{3.5mm} where $\Theta$ represents the trainable parameters, and $N$ is the sequence length. 
...
In this section, we discuss the evaluation results, both quantitatively and qualitatively. More specifically, some query benchmarking are provided for selected questions. The generated rules are then interpreted, followed by decision reasoning with rules. For pre-training of \textit{BERT}-based NER models, the maximum input length is set to $256$. During the fine-tuning for 20 epochs, the training set is shuffled for each epoch, gradient clipping is applied, initial learning rate is set to $2 e^{-5}$, and the Adam optimizer is used to optimize the loss~(i.e., \cref{eq:bert_class_loss}) with the scheduled learning rate. \Cref{table:bert_params} shows the hyperparameter combination used for pretraining and finetuning phases. 
...
\hspace*{3.5mm} Prot{\'e}g{\'e} 5.5.0 is used for the reasoning, w.r.t DLx querying. The DLx rule generations with Pellet, ELK, and HermiT reasoners. 
%The detail information including versions are shown in \cref{fig:plugins_info}. 
However, since ELK does not support object all values from axioms, while HermiT gave some inconsistent~(giving different number of rules), we report the rules generated with Pellet reasoner only. The inferred rules based on the reasoning are expressed in DLx format. To evaluate the effectiveness of domain-specific potentials of transformer-based approach for information extraction, a comparative analysis with the well-established CRF and Bi-LSTM architectures. 
...
%We interpret the inferred rules. We apply 
The DLx Class Axiom Renderer Plugin\footnote{\url{https://github.com/MindfulMichaelJames/ProtegeDLAxiomPlugin}} is used to represent the rules.   
...
\noindent i) If \texttt{<Biomarker> $\sqsubseteq$ $\forall$is\_a.Protein-coding}, following two rules implied:\\
...
\noindent ii) If \texttt{<Biomarker> $\sqsubseteq$ $\forall$is\_a.POTSF}, following two rules implied:\\
...
\vspace{-6mm}
\noindent iii) If \texttt{<Biomarker>$\sqsubseteq$ $\forall$is\_a.Oncogene}, following two rules implied:\\
...
\vspace{-6mm}
\noindent iv) For every disease \texttt{responsible\_for<cancer-type>}, the following three rules are implied:\\
...
\subsubsection{Rules associated for subclasses}
Following rules are generated for features:
...
Following rules are generated for biomarker's significance:
...
Following rules are generated for biomarker types:
...
Following rules are generated for evidence types:
...
Following are the rules generated for the cancer types:
