...
\hspace*{3.5mm} %A well-trained model is subject to enough data as DNNs require a lot of labeled training data. 
...
The problem of cancer sub-typing is to observe which genetic mutations are responsible for which breast cancer subtypes, based on multimodal genomics data containing ER, PGR, and HER2/neu statuses of breast cancer patients, either separately or in a multimodal way. The heterogeneity of expression across samples is captured in a gene regulation space, instead of gene expression space. This also involves transforming gene expression, miRNA expression, and DNA methylation profiles into regulation profiles for characterizing differential expression of genes between different subtypes. Since breast cancer subtype classification consists of following three sub-tasks based on ER, PGR, and HER2/neu status: %, each of these will correspond to different neural networks: 
...
\begin{table}[h]
    \scriptsize
    \caption[Statistics of the data used for breast cancer sub-type classification]{Statistics of the data used for breast cancer sub-type classification~\cite{karimACCESS2019}}
    	\vspace{-2mm}
...
\begin{table}[h]
    \centering
    \footnotesize
    \caption[Top results for ER, PGR, and HER2/neu status classification]{Top results for ER, PGR, and HER2/neu status classification~\cite{karimACCESS2019}}
    \vspace{-2mm}
...
\begin{figure*}[h]
	...
	\end{subfigure}
	\caption[Confusion matrix for ER, PGR and HER2 status classification]{Confusion matrix for ER, PGR and HER2 status classification~\cite{karimACCESS2019}} 
	\label{fig:multi_cms}
\end{figure*}
...
\subsection{Qualitative analysis of the learned representation}
...
\begin{table*}
	...
	\caption[Subtypes prediction with ML classifiers]{Subtypes prediction with ML classifiers~(*=modality with best results)~\cite{karimACCESS2019}}
...
\begin{figure*}
	...
	\begin{subfigure}{0.49\linewidth}
        ...
        \label{fig:top_her2}
	\end{subfigure}
	\caption[Top ER, PGR, and HER2 status classifications results for each input type]{Top ER, PGR, and HER2 status classifications results for each input type~\cite{karimACCESS2019}} 
	\label{fig6}
	\vspace{-2mm} 
\end{figure*}
