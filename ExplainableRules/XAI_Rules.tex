...
\small{
    \DontPrintSemicolon \SetKwInOut{Input}{Input}%
      ...
      \Input{Dataset $X$- used for training the `black-box' model or a new dataset from the same distribution.}%
      \Output{A surrogate model $M_s$ to approximate `black-box' model.}%
      \BlankLine%
     \For{for each sample $x \in X$}{ \tcp*{for each sample in training set do}
            %$\mathcal{(A}, \mathcal{C}) \leftarrow  \{\}$ \tcp*{Get the predictions of the `black-box' model}\;
            \vspace{-2mm}
            Step -1: select an interpretable model type\tcp*{e.g., linear model, decision tree}\;
            Step-2: train the interpretable model $M_s$ on dataset $X$\;
            Step-3: generate the predictions $Y_M$ from surrogate model\;
...
\hspace*{3.5mm} where $\hat{y}_{*}^{(i)}$ is the prediction for the $i^{th}$ instance of the surrogate model, $\hat{y}^{(i)}$ the prediction of the `black-box' model, $\overline{\hat{y}}$ the mean of the `black-box' model predictions, $SSE$ is the sum of squares error, and $SST$ is the sum of squares total. The $R^2$ can be interpreted as the percentage of variance captured by the surrogate model, i.e., $R^2$ is close to $1(=$ low $\mathrm{SSE}$), then the interpretable model
approximates the behavior of the `black-box' model very well~\cite{molnar2019interpretable}. Subsequently, we even can replace the complex model with the interpretable model. However, if $R^2$ is close to 0~(i.e., high SSE), then the interpretable model fails to explain the `black-box' model and should not replace. 
